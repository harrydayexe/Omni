...
func handleUpdatePost(logger *slog.Logger, postRepo storage.Repository[models.Post]) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		err = postRepo.Update(r.Context(), newPost)
		var e *storage.NotFoundError
		if errors.As(err, &e) {
			logger.DebugContext(r.Context(), "post does not exist", slog.Any("id", newPost.Id()))
			var errorMessage = `{"error":"Not Found","message":"Post with that ID could not be found to update."}`
			w.WriteHeader(http.StatusNotFound)
			w.Write([]byte(errorMessage))
			return
		}
		if err != nil {
			logger.ErrorContext(r.Context(), "failed to update post in db", slog.Any("error", err))
			w.WriteHeader(http.StatusInternalServerError)
			return
		}
